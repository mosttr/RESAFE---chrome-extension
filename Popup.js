...
function updateButtonColors() {
  toggleCameraBtn.style.backgroundColor = cameraEnabled ? 'green' : 'red';
  toggleMicBtn.style.backgroundColor = micEnabled ? 'green' : 'red';
}

// Send message to content scripts
function sendToggleMessage(type, state) {
  chrome.tabs.query({ active: true, currentWindow: true }, ([tab]) => {
    if (!tab?.id) return;
    chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: (t, s) => {
        window.ReSAFE = window.ReSAFE || {};
        window.ReSAFE[t] = s;

        if (!s) {
          navigator.mediaDevices.getUserMedia = async () => {
            throw new Error("Camera/Mic access blocked by ReSAFE");
          };
        } else {
...
toggleCameraBtn.addEventListener('click', () => {
  ...
  updateButtonColors();
  sendToggleMessage('camera', cameraEnabled);
});

toggleMicBtn.addEventListener('click', () => {
  micEnabled = !micEnabled;
  updateButtonColors();
  sendToggleMessage('mic', micEnabled);
});
